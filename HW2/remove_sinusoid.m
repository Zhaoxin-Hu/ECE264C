...
function [windowed_signal_component, windowed_noise_component] = ...
    ...
    denominator = (mean(window))^2 - mean(window'.*(W.^(-2 .* n))) *    mean(window'.*(W.^(2 .* n)));
    Y1 = (mean(windowed_data_segment .* (W.^(-n))') * mean(window) -    mean(windowed_data_segment .* (W.^(n))') * mean(window' .* (W.^(-2 .* n)))) /    denominator;
    Ym1 = (mean(windowed_data_segment .* (W.^(n))') * mean(window) -    mean(windowed_data_segment .* (W.^(-n))') * mean(window' .* (W.^(2 .* n)))) /    denominator;
    ...
    windowed_noise_component = windowed_noise_component_with_DC -    window .* mean(windowed_noise_component_with_DC) / mean(window);
